...
class Settings
{
    ...
    public static function sanitize(array $input): array
    {
        ...
        $branch = sanitize_text_field(trim($input['branch'] ?? '')) ?: $defaults['branch'];
        $baseFolder = sanitize_text_field(trim($input['base_folder'] ?? '', "/ \t\n\r\0\x0B")) ?: $defaults['base_folder'];
    ...
    {
        add_options_page(
            __('Export posts to GitHub: Settings', 'posts-to-github-md'),
            __('Posts to GitHub', 'posts-to-github-md'),
            'manage_options',
            'potogh-settings',
            [$this, 'renderPage']
        );
    ...
    public function renderPage(): void
    {
        ...
        <div class="wrap potogh-settings">
            <h1><?php echo esc_html__('Export posts to GitHub: Settings', 'posts-to-github-md'); ?></h1>
            <p>
                <?php
                printf(
                    /* translators: %s: link to the Export posts screen under the Posts menu */
                    esc_html__('Looking to export posts? Head over to %s.', 'posts-to-github-md'),
                    '<a href="' . esc_url(ExportTab::pageUrl()) . '">' . esc_html__('Posts &rsaquo; Export to GitHub', 'posts-to-github-md') . '</a>'
                );
                ?>
            </p>
        ...
        <div class="potogh-credits">
            <p class="description">
                ...
                printf(
                    ...
                    esc_html__('Posts to GitHub by %1$s, source on %2$s.', 'posts-to-github-md'),
        ...
        ?>
            <form method="post" action="options.php">
                ...
                <table class="form-table">
                    <tr>
                        <th><label for="potogh_token"><?php esc_html_e('GitHub Personal Access Token', 'posts-to-github-md'); ?></label></th>
                        <td>
                            ...
                            <p class="description">
                                ...
                                printf(
                                    ...
                                    esc_html__('A fine-grained or classic token with %1$s access to the target repository. Generate one at %2$s.', 'posts-to-github-md'),
                                    '<code>repo</code>',
                    ...
                    <tr>
                        <th><label for="potogh_owner_repo"><?php esc_html_e('Repository', 'posts-to-github-md'); ?></label></th>
                        <td>
                            ...
                            <?php if ($repoLocked) : ?>
                                <button type="button" class="button" id="potogh-edit-repo"><?php esc_html_e('Change repository', 'posts-to-github-md'); ?></button>
                            <?php endif; ?>
                            <button type="button" class="button" id="potogh-test-connection"><?php esc_html_e('Test connection', 'posts-to-github-md'); ?></button>
                            <span id="potogh-test-connection-result"></span>
                            <p class="description">
                                <?php esc_html_e('Enter either "owner/repo" or the full GitHub URL (e.g. https://github.com/owner/repo).', 'posts-to-github-md'); ?>
                                <?php if ($repoLocked) : ?>
                                    <?php esc_html_e('Locked to avoid accidental changes — click "Change repository" to edit it.', 'posts-to-github-md'); ?>
                                <?php endif; ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="potogh_branch"><?php esc_html_e('Branch', 'posts-to-github-md'); ?></label></th>
                        <td>
                            ...
                            <button type="button" class="button" id="potogh-detect-branch"><?php esc_html_e('Detect from repository', 'posts-to-github-md'); ?></button>
                            <span id="potogh-detect-branch-result"></span>
                            <p class="description">
                                <?php esc_html_e('The branch posts will be committed to (e.g. main).', 'posts-to-github-md'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="potogh_base_folder"><?php esc_html_e('Base folder', 'posts-to-github-md'); ?></label></th>
                        <td>
                            ...
                            <p class="description">
                                <?php esc_html_e('Repository folder posts are exported into (e.g. posts). Left empty, it defaults to "posts".', 'posts-to-github-md'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Automatic export', 'posts-to-github-md'); ?></th>
                        <td>
                            <label for="potogh_auto_export">
                                ...
                                <?php esc_html_e('Automatically export new posts to GitHub when published.', 'posts-to-github-md'); ?>
                            </label>
                            <p class="description">
                                <?php esc_html_e('Runs a few seconds after publishing via WP-Cron, without delaying the Publish button. Existing posts are not exported retroactively — use the Export posts page for those.', 'posts-to-github-md'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Automatic re-export', 'posts-to-github-md'); ?></th>
                        <td>
                            <label for="potogh_auto_reexport">
                                ...
                                <?php esc_html_e('Automatically re-export already-published posts to GitHub when updated.', 'posts-to-github-md'); ?>
                            </label>
                            <p class="description">
                                <?php esc_html_e('Same background behavior as automatic export, triggered on every update to a published post instead of only its first publish.', 'posts-to-github-md'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Bulk export method', 'posts-to-github-md'); ?></th>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            <label for="potogh_cleanup_on_uninstall">
                                ...
                                <?php esc_html_e('Remove all plugin settings and export data when this plugin is deleted.', 'posts-to-github-md'); ?>
                            </label>
                            <p class="description">
                                <?php esc_html_e('Uncheck to keep your GitHub connection settings and per-post export history in the database if you delete the plugin (useful before reinstalling).', 'posts-to-github-md'); ?>
                            </p>
                        </td>
                    </tr>
                </table>
                <p class="submit potogh-submit-row">
                    ...
                    <button type="submit" class="button button-primary" id="potogh-save-settings" <?php disabled(!self::isConfigured()); ?>>
                        <?php esc_html_e('Save Changes', 'posts-to-github-md'); ?>
                    </button>
                </p>
            </form>
        <?php
    ...
    public function handleAjaxTestConnection(): void
    {
        ...
        if ($sanitized['token'] === '' || $sanitized['owner_repo'] === '') {
            wp_send_json_error(['message' => __('Enter both a token and a valid repository before testing the connection.', 'posts-to-github-md')], 400);
        }
    ...
    public function handleAjaxDetectBranch(): void
    {
        ...
        if ($sanitized['token'] === '' || $sanitized['owner_repo'] === '') {
            wp_send_json_error(['message' => __('Enter both a token and a valid repository first.', 'posts-to-github-md')], 400);
        }
