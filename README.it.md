...
Ogni campo ha un testo di aiuto sotto l'input con indicazioni sul formato atteso. Il pulsante **"Save Changes"** richiede un **"Test connection"** riuscito (controllo di sola lettura, non scrive nulla sul repository) solo la prima volta che configuri token/repository, oppure ogni volta che modifichi token, repository o branch in seguito; cambiare altre impostazioni (export automatico, metodo di bulk export, ecc.) non lo ridisabilita.
