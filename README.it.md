...
Ogni campo ha un testo di aiuto sotto l'input con indicazioni sul formato atteso. Il pulsante **"Test connection"** si trova accanto al campo Repository (controllo di sola lettura, non scrive nulla sul repository). Una volta configurato un repository, quel campo viene bloccato per evitare modifiche accidentali — clicca **"Change repository"** accanto ad esso per sbloccarlo e modificarlo. Il pulsante **"Save Changes"** richiede un test riuscito solo la prima volta che configuri token/repository, oppure ogni volta che modifichi token, repository o branch in seguito; cambiare altre impostazioni (export automatico, metodo di bulk export, ecc.) non lo ridisabilita.
