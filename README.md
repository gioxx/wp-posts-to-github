...
1. Copy the whole plugin folder to `wp-content/plugins/posts-to-github-md/` on your WordPress site (the `vendor/` folder with dependencies is already included: no need to run Composer).
2. Go to **Plugins** in the WordPress dashboard and activate "Posts to GitHub".
...
Go to **Settings → Posts to GitHub** and fill in:
...
Each field has a help text under the input describing the expected format. The **"Test connection"** button sits next to the Repository field (read-only check, it never writes to the repository). Once a repository is configured, that field is locked to avoid accidental changes — click **"Change repository"** next to it to unlock and edit it. The **"Save Changes"** button only requires a successful test the first time you configure the token/repository, or whenever you edit the token, repository or branch afterward — changing any other setting (automatic export, bulk export method, etc.) doesn't re-lock Save.
...
3. Use the filters above the table to narrow down the list: status, search, category, tag and publish month all sit on one row, plus a per-page dropdown (10/25/50/100, remembered for next time). Filtering reloads the list, same as WordPress' own post list, with pagination shown both above and below the table. The category and tag dropdowns show a live post count next to each term, updated to match whatever other filters are currently active.
...
Below the posts table, the **"Taxonomy backup"** section lets you push a JSON snapshot of your categories and tags to the same GitHub repository: `wp-categories.json` and `wp-tags.json`, written to the repository root regardless of the configured base folder. WordPress stays the source of truth — the plugin only writes to GitHub here, it never reads categories or tags back from the repository.

The status line tells you whether anything changed since the last upload (new/removed categories or tags), based on a hash stored locally in the database — no extra GitHub API calls just to show that status. Click **"Update categories and tags on GitHub"** to push the current state in a single commit; the button stays enabled even when nothing changed, in case you want to force a commit anyway. Direct links to both files on GitHub sit right below the button.
